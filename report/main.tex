...
To combat this particular problem, the pre-processing pipeline will use stratified split, so that all the training, validation and test sets ahve the same class distribution. 
In addition, we implement class weights so bonafide weighs 9x higher compared to spoof to compensate for the imbalance.

All audio files are sampled at 16 kHz. After studying the duration of all the samples on the dataset, we decided to crop or padd to 4 seconds. This makes the rest of the pipeline easier to work with, as handling variable input size takes more effort. 4 seconds, or 200 frames captures the 95th percentile or all the audio sample lengths. 
...
For the Bi-LSTM model, we extract Mel-frequency cepstral coefficients (MFCCs). We compute 13 static MFCC coefficients. We tried implementing the first-order (delta) and second-order (delta-delta) derivatives, to resulting in 39 features per time frame, but was droped for the sake of simplicity.
...
We don't do crop augmentation, because we are using fixed padding as the input. Its important to note that this augmentation is done before the normalization. 
...
where $w_{y_i}$ is the weight for the true class of sample $i$. We set $w_{\text{bonafide}} = 5.0$ and $w_{\text{spoof}} = 0.56$, forcing the model to pay substantially more attention to bonafide samples while preventing complete disregard of the majority class.
...
\midrule
\multirow{2}{*}{CNN} & Bonafide & -- & -- & -- \\
                     & Spoof & -- & -- & -- \\
\midrule
\multirow{2}{*}{ViT} & Bonafide & -- & -- & -- \\
                     & Spoof & -- & -- & -- \\
\midrule
\multirow{2}{*}{Bi-LSTM} & Bonafide & -- & -- & -- \\
                         & Spoof & -- & -- & -- \\
\bottomrule
\end{tabular}
\label{tab:per_class}
...
The severe class imbalance (90/10 split) requires careful consideration. Our weighted loss approach helps prevent the model from simply predicting the majority class. However, this introduces a hyperparameter (the weight ratio) that must be tuned. Alternative approaches such as oversampling, undersampling, or focal loss could be explored in future work.
...
\begin{itemize}
    \item \textbf{Ensemble methods}: Combining predictions from multiple models to leverage their complementary strengths
    \item \textbf{Multi-scale analysis}: Processing audio at multiple temporal resolutions
    \item \textbf{Transfer learning}: Leveraging pre-trained models from related audio tasks
    \item \textbf{Real-time optimization}: Reducing computational requirements for deployment
    \item \textbf{Cross-dataset evaluation}: Testing generalization to other datasets and synthesis methods
    \item \textbf{Explainability}: Developing methods to interpret and explain model decisions
    \item \textbf{Multi-class classification}: Identifying specific synthesis techniques
\end{itemize}
